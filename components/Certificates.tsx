...
const certificatesList = [
  ...
  },
  {
    ...
    skillsVerified: ["Data Analytics", "AI Orchestration", "Predictive Analytics", "Data Operations"]
  }
];
...
export default function Certificates() {
  return (
    <section id="certificates" className="py-20 relative font-mono">
      ...
      {/* Grid structure updated to handle 3 columns cleanly on larger screens */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto px-4">
        {certificatesList.map((cert, idx) => (
          /* Wrapped exactly in TiltCard to force 3D Axis Matrix */
          <TiltCard key={idx} className="w-full">
            <div className="group relative p-6 rounded-md border border-purple-500/20 bg-purple-950/5 hover:border-cyan-400 transition-all duration-300 overflow-hidden h-full flex flex-col justify-between">
              ...
              <div className="flex gap-4 items-start relative z-10 h-full">
                ...
                <div className="flex-1 space-y-3 flex flex-col justify-between h-full">
                  <div className="space-y-1">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="text-sm font-bold text-white tracking-tight group-hover:text-cyan-400 transition-colors line-clamp-2">
                        {cert.title}
                      </h3>
                    ...
                    <p className="text-[11px] text-gray-400 font-medium">
                      {cert.issuer} • <span className="text-purple-400 font-mono font-semibold">{cert.date}</span>
                    </p>
                  </div>
